...
- [ ] Step 1: Add failing schema/introspection tests that assert legacy auth mutations are absent from the schema
- [ ] Step 2: Run focused Accounts GraphQL tests to verify RED
- [ ] Step 3: Remove `loginWithPassword`, `requestMagicLinkLogin`, and `loginWithMagicLink` from the schema definition
- [ ] Step 4: Keep generic auth entrypoint coverage green without reintroducing legacy transport shapes
...
- [ ] Step 1: Write the contract doc for the supported mobile GraphQL surface
- [ ] Step 2: Update the roadmap to record delivered viewer-scoped social reads and auth-surface cleanup
- [ ] Step 3: Update the plan index so active work reflects the new slices instead of stale completed entries
- [ ] Step 4: Run final verification on touched GraphQL suites plus `mix compile` and `mix typecheck`
- [ ] Step 5: Update checklist progress and commit milestone
