...
- Batch: `GQL-009` Stage 8 Task 4 auth/recovery/token resolver extraction
...
- Current status:
  ...
  - `GQL-009`: Stage 7 complete; Stage 8 Task 1 contact resolver extraction complete; Stage 8 Task 2 data-governance resolver extraction complete; Stage 8 Task 3 user/profile/identity resolver extraction complete; Stage 8 Task 4 auth/recovery/token resolver extraction is next.
...
- Execute `GQL-009` Stage 8 Task 4 by extracting auth, recovery, and token responsibilities into a focused auth resolver: auth challenge/sign-up/login, viewer token issue/refresh/revoke, password reset request/reset, deterministic auth URL builders, auth field validation, token projection, and auth-entry payload projection. Preserve the public GraphQL contract and use TDD for production-code refactors.
...
- Continue with `GQL-009` Stage 8 Task 4: auth/recovery/token resolver extraction.
