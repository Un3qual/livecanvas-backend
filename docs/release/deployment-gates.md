...
| Runtime failover rehearsal gate | Release Engineer + Live Runtime Owner | `MIX_ENV=test mix release.live_runtime_drill --session-id <id> --takeover-node <node> --confirm` | Command exits `0`; all five shard-owner drill steps complete and reconnect probe succeeds without ghost participants. | P1 |
