...
| Gate order audit (dry run) | Release Engineer | `mix release.gates --dry-run` and `MIX_ENV=test mix release.migration_drill --dry-run --step 1` | Output lists deterministic ordered command plan that matches this runbook. | P1 |
...
3. Run the gate order audit and attach output to the release ticket.
4. Confirm working tree integrity.
5. IC marks release as `GO` only after all P0/P1 blockers are cleared.
