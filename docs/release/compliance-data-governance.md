...
| Live | `live_sessions`, `live_participants`, `live_session_runtime_owners` | Include sessions hosted by requester and participation records tied to requester. | Rows remain untouched while account-deletion hard purge is stubbed. | `live_participants` retained 180 days; `live_sessions` retained 365 days unless required longer by moderation incidents. | Hold skips live-session/live-participant retention purge and deletion purge. |
