...
- [ ] Add failing GraphQL tests proving an owned unmatched contact succeeds while arbitrary, foreign, self-owned, and newly matched rows create neither token nor email.
- [ ] Add failing token tests proving legacy-context credentials are rejected and fragment-context credentials are accepted.
- [ ] Change the Relay input to `contact_match_id`, decode it as `:contact_match`, and move the ownership/match/recipient decision into `LC.Accounts`.
- [ ] Add `contact_invite_fragment_token` to the PostgreSQL enum migration and schema enum; issue and validate only that context.
- [ ] Add indexes on `inviter_id` and `recipient_user_id` in the conversion migration.
- [ ] Run the focused Accounts, token, mutation, and rate-limit tests; run `mix format` and commit.
...
- [ ] Add a failing query-capture test requiring `FOR UPDATE` on the ownership lookup.
- [ ] Add a failing integration test that records distinct `pg_backend_pid()` values and proves two consumers receive one conversion.
- [ ] Lock the matching ownership join for the remainder of the consumption transaction.
- [ ] Replace the shared-connection task test with the integration test and scope all conversion assertions by token ID.
- [ ] Run both focused tests and commit with the Task 1 milestone if they remain one coherent backend change.
...
- [ ] Add failing backend tests for port `99999` and `.invalid.` hosts.
- [ ] Add failing mobile tests for missing production origin and a Babel transform that must contain the supplied production origin.
- [ ] Tighten backend origin predicates without introducing a second config abstraction.
- [ ] Replace dynamic `globalThis.process` access with direct Expo-inlineable reads and allow localhost fallback only outside production.
- [ ] Run focused backend/mobile config tests and commit with the appropriate backend/mobile milestone.
...
- [ ] Add failing routing tests for trailing-slash, nested, and encoded invite paths containing raw tokens.
- [ ] Add failing screen tests proving `unauthenticated` and unknown errors retain the handoff while only `invalid_contact_invite` clears it.
- [ ] Broaden candidate recognition while keeping exact route acceptance strict.
- [ ] Map backend error codes explicitly and preserve the existing attempt-ID stale-callback guarantees.
- [ ] Run focused Bun and Jest suites, generate Relay artifacts after the delivery input change, and commit.
...
- [ ] Add a failing controller assertion that the landing page references only the dedicated entry bundle and has no CSRF meta tag.
- [ ] Build both JavaScript entry points, remove landing initialization from `app.js`, and update the template.
- [ ] Run landing parser/controller tests and `mix assets.build`.
- [ ] Run `mix precommit`, `mix typecheck`, `mix slop.changed`, mobile `test:quality`, Relay generation, and `git diff --check`.
- [ ] Re-read this plan and the remediation spec, update their checkboxes, commit the final verified milestone, and push the branch.
