...
**Architecture:** Accounts owns opaque SHA3-hashed invite-token validation and transactional consumption. Delivery begins from a viewer-owned unmatched contact Relay ID, which Accounts locks and re-evaluates before deriving the eligible recipient; the API never accepts an arbitrary recipient address. Emailed HTTPS links keep the raw token in a URL fragment, so it is never sent to Phoenix, proxies, request telemetry, or referrers; a neutral public landing page uses a dedicated static entry bundle to translate that fragment into an explicit LiveCanvas deep link. Authenticated Relay consumption requires the viewer to own the normalized recipient email. A matching verified email consumes directly; for password sign-up, possession of the recipient-bound invite token atomically verifies only that matching email and confirms the account before recording conversion and deleting the token. The conversion retains the token's SHA3 hash so the same viewer can safely read back the committed result when a response is lost, without making token UUID possession sufficient. Mobile preserves the token through sign-in/sign-up and exposes delivery state only after the route contract is configured.
...
- [x] Add 7-day `valid_contact_invite_token?/2` validation requiring secure hash match and exact `:contact_invite_fragment_token` context; reject the legacy `:contact_invite_token` context after the fragment cutover.
...
- [x] Add a pure, unit-tested fragment parser and import its guarded landing-page initializer from a dedicated `assets/js/contact_invite_landing_entry.js` bundle. It reads exactly one nonblank `token` value from `window.location.hash`, sets the explicit `livecanvas-mobile://invite?token=<encoded-token>` action, and otherwise leaves the page in one generic invalid-or-expired state. Do not interpolate the fragment into server-rendered HTML or load the Phoenix/LiveView app bundle on this public page.
...
- [x] Configure and validate `EXPO_PUBLIC_APP_ORIGIN` independently from the API origin, using direct Expo-inlineable environment reads and the same HTTPS origin restrictions and trailing-slash normalization as the backend; require it explicitly for production builds. In async startup bootstrap, accept exactly one nonblank token from `livecanvas-mobile://invite?token=...` or an exact HTTPS `/invites#token=...` fragment whose origin matches that configured public app origin, persist it before routing, and replace it with `/invite?handoff=<opaque-id>`. Wrong-origin or malformed invite-shaped links and storage failures fall back to token-free `/invite`; unrelated links remain unchanged. Do not retain the raw invite URL or token in `StartupSnapshot.initialUrl`, `initialHref`, navigation state, telemetry, or error text.
...
- [x] Show Send invite only for an unmatched row with a normalized `inviteRecipient`; submit the opaque contact-match Relay ID so Accounts can re-authorize and derive the recipient, while matched rows continue to expose profile navigation only.
...
- [x] Review remediation makes delivery contact-driven and server-authorized, versions fragment-era credentials, locks recipient ownership, adds the missing conversion foreign-key indexes, and replaces the sandbox-shared concurrency test with two verified independent PostgreSQL connections.
- [x] Fresh final verification: test database reset and all migrations pass; `mix test` reports 1,013 tests with 0 failures and 1 excluded; `mix typecheck`, warnings-as-errors compilation, `mix check.typespecs`, focused Elixir formatting, `mix assets.build`, and the 6-test landing parser suite pass. Relay schema and artifacts regenerate cleanly; `bun run test:quality` passes both typechecks, lint, 551 Bun tests, and 163 Jest tests. Root `git diff --check` passes.
- [x] Aggregate quality-gate limitations are recorded rather than hidden: `mix precommit` stops on a pre-existing global ExDNA budget failure (6 clones against 5, all outside this change), while `mix slop.changed` reports no changed-code Credo issues before its Reachability pass stalls without output and is stopped after a bounded wait.
