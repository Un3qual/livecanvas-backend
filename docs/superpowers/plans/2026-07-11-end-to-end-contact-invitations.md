...
- [x] Fresh final verification: test database reset and all migrations pass; `mix test` reports 1,010 tests with 0 failures and 1 excluded; `mix typecheck`, warnings-as-errors test compilation, `mix check.typespecs`, branch-changed Elixir formatting, `mix assets.build`, and the 6-test landing parser suite pass. The mobile schema and operations were unaffected, so Relay regeneration was not required; `bun run test:quality` passes both typechecks, lint, 544 Bun tests, and 160 Jest tests. Root `git diff --check` passes.
