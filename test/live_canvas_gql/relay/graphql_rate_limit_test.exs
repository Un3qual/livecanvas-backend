defmodule LCGQL.Relay.GraphQLRateLimitTest do
  ...
  test "uses moderation-action limits for reportPost", %{conn: conn} do
    previous_config = Application.get_env(:live_canvas, LC.RateLimiter, [])
    ...
    moderation_mutation = """
    mutation {
      reportPost(input: { postId: \"123\", reason: OTHER }) {
        errors {
          message
        }
      }
  ...
  test "uses moderation-action limits for decidePostReport", %{conn: conn} do
    previous_config = Application.get_env(:live_canvas, LC.RateLimiter, [])
    ...
    moderation_mutation = """
    mutation {
      decidePostReport(input: { reportId: \"123\", status: DISMISSED }) {
        errors {
          message
        }
      }
