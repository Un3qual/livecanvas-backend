defmodule LCGQL.Live.ResolverTest do
  ...
  describe "live_session_channel_topic/3" do
    ...
    test "returns an opaque topic for an active host on a followers-only session" do
      host = user_fixture()
      {:ok, session} = Live.start_live_session(host, %{visibility: :followers})
      {:ok, live_session} = Live.mark_session_live(session)

      assert {:ok, topic} =
               Resolver.live_session_channel_topic(live_session, %{}, resolution_for(host))

      assert topic == LiveSessionTopics.live_session_topic(live_session.id)
    end

    test "accepts a viewer scope user map with an integer id" do
      host = user_fixture()
      viewer = user_fixture()
      _follow = accepted_follow_fixture(viewer, host)
      {:ok, session} = Live.start_live_session(host, %{visibility: :followers})
      ...
      assert {:ok, topic} =
               Resolver.live_session_channel_topic(
                 live_session,
                 %{},
                 id_map_resolution_for(viewer)
               )
  ...
  defp id_map_resolution_for(viewer) do
    %Absinthe.Resolution{context: %{current_scope: %{user: %{id: viewer.id}}}}
  end
end
